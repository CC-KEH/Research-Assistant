...
class ShowPdf():
    ...
    def pdf_view(self, master, width=1200, height=600, pdf_location="", bar=True, load="after", dpi=100):
        ...
        def add_img(night_mode=False):  # Added night_mode argument
            # Clear previous PDF images
            self.img_object_li.clear()
            self.tkimg_object_li.clear()

            # Clear the Text widget
            self.text.configure(state="normal", bg="black" if night_mode else "white")
        ...
        def zoom_in(event=None):
            ...
            for i, im in enumerate(self.img_object_li):
                self.tkimg_object_li[i] = ImageTk.PhotoImage(im.resize((int(res*self.tkimg_object_li[i].width()), int(res*self.tkimg_object_li[i].height())), Image.Resampling.LANCZOS))
